...
func TestGenKeySchedule(t *testing.T) {
	t.Run("鍵スケジュールの生成が正しい", func(t *testing.T) {
		...
		key, _ := tls.KeyScheduleFactory("SHA256", clientHelloRaw, serverHelloRaw, sharedkey)
		// if err != nil {
		...
		t.Logf("EarlySecret: %x", key.EarlySecret)
		t.Logf("HandshakeSecret: %x", key.HandshakeSecret)
		...
		t.Logf("ClientFinishedKey: %x", key.ClientFinishedKey)
		t.Logf("ServerFinishedKey: %x", key.ServerFinishedKey)
		t.Logf("EarySecretState: %x", key.EarySecretState)
		t.Logf("HandshakeSecretState: %x", key.HandshakeSecretState)
		t.Logf("MasterSecret: %x", key.MasterSecret)
	})
}
